...
from .preprocessing import RandomTranslateCrop
from .aircraft_ship import AircraftShip

__factory = {
    "HOSS": HOSS,
    "Pretrain": Pretrain,
    "AircraftShip": AircraftShip, # <--- 新增
}
...
def train_collate_fn(batch):
    ...
    # 返回camids，便于forward区分SAR
    return torch.stack(imgs, dim=0), pids, camids, viewids, img_size, camids
...
def val_collate_fn(batch):
    ...
    img_size = torch.tensor(img_size, dtype=torch.float32)
    # 返回camids_batch，便于forward区分SAR
    return torch.stack(imgs, dim=0), pids, camids, camids_batch, viewids, img_paths, img_size


def make_dataloader(cfg):
    # cfg.INPUT.SIZE_TRAIN is specified as [width, height] in config; torchvision expects (height, width)
    train_size = (cfg.INPUT.SIZE_TRAIN[1], cfg.INPUT.SIZE_TRAIN[0])
    test_size = (cfg.INPUT.SIZE_TEST[1], cfg.INPUT.SIZE_TEST[0])

    train_transforms = T.Compose(
        [
            T.Resize(train_size, interpolation=3),
            RandomTranslateCrop(cfg.INPUT.SIZE_TRAIN, max_translate=getattr(cfg.INPUT, 'MAX_TRANSLATE', 0.2)),
