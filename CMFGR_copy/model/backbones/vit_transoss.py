...
class TransOSS(nn.Module):
    ...
    def load_param(self, model_path):
        ...
        # helper to normalize keys and generate candidates
        def norm_key(k):
        ...
        # Build a mapping from checkpoint keys -> model keys where possible
        ...
        # Copy parameters using remapped keys
        for k, v in param_dict.items():
            key = remap.get(k, None)
            if key is None:
                # skip keys that don't map to current model
                # but keep compatibility messages
                # print(f'{k} not mapped to any model key, skipping.')
                continue
            ...
            # handle old patch_embed shape
            if 'patch_embed.proj.weight' in key and len(v.shape) < 4:
                O, I, H, W = self.patch_embed.proj.weight.shape
                v = v.reshape(O, -1, H, W)
            elif key == 'pos_embed' and v.shape != self.pos_embed.shape:
                # To resize pos embedding when using model at different size from pretrained weights
                # allow checkpoints that used distilled tokens
                try:
            ...
            try:
                if key in self.state_dict():
                    ...
                    print(f'{key} not found in the model state_dict, skipping.')
            except Exception:
                print('===========================ERROR=========================')
                if 'mie_embed' in key:
                    print(f'{key} not in the model')
                    continue
                if key in self.state_dict():
                    print('shape do not match in k :{}: param_dict{} vs self.state_dict(){}'.format(key, v.shape, self.state_dict()[key].shape))
                else:
                    print(f'Failed to load param {key} and key not present in model state_dict')
